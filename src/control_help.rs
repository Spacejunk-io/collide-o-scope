...
pub static CONTROL_HELP: &[HelpEntry] = &[
    ...
    entry(
        Temporal,
        "loom_topology",
        "The geometry the Loom folds history through: linear, radial, spiral, contour, folded, or kaleidoscopic.",
    ),
    entry(
        Temporal,
        "loom_interpolation",
        "Whether the Loom's history read snaps to a layer or interpolates between two.",
    ),
    entry(
        Temporal,
        "loom_depth",
        "How deep into the history ring the Loom reaches.",
    ),
    entry(
        Temporal,
        "loom_phase",
        "Where the Loom's pattern sits along its own cycle.",
    ),
    entry(
        Temporal,
        "loom_scale",
        "How large the Loom's geometry is relative to the frame.",
    ),
    entry(
        Temporal,
        "loom_angle",
        "The Loom geometry's rotation, in degrees.",
    ),
    entry(
        Temporal,
        "loom_folds",
        "How many repeats the Loom's fold produces.",
    ),
    entry(
        Temporal,
        "loom_quantization",
        "Snaps the Loom's history read to discrete steps; zero leaves it continuous.",
    ),
    entry(
        Temporal,
        "atlas_amount",
        "How strongly the Atlas partitions the frame into independently-timed territories.",
    ),
    entry(
        Temporal,
        "atlas_seed",
        "Selects the Atlas territory arrangement. It is authored identity, not a value: Morph recalls it as an endpoint rather than interpolating an RNG.",
    ),
    entry(
        Temporal,
        "atlas_territories",
        "How many territories the Atlas divides the frame into.",
    ),
    entry(
        Temporal,
        "atlas_collision",
        "How much neighbouring territories bleed into one another at their borders.",
    ),
    entry(
        Temporal,
        "garden_amount",
        "How strongly the Refresh Garden holds and releases regions of the image.",
    ),
    entry(
        Temporal,
        "garden_gate",
        "What decides a Garden region refreshes: temporal delta, luma, chroma, a cellular ridge, audio energy or onset, a matte, or motion.",
    ),
    entry(
        Temporal,
        "garden_threshold",
        "How much of the gate signal is needed before a region refreshes.",
    ),
    entry(
        Temporal,
        "garden_softness",
        "Feathers the Garden's gate decision, so regions ease between held and refreshing rather than switching hard.",
    ),
    entry(
        Temporal,
        "garden_decay",
        "How quickly a held Garden region gives way once its gate closes.",
    ),
    entry(
        Temporal,
        "garden_max_hold_ticks",
        "A hard ceiling on how long any region may hold, so nothing freezes permanently.",
    ),
    entry(
        Temporal,
        "score_enabled",
        "Arms the Collision Score, which advances temporal state through a sequence of discrete states.",
    ),
    entry(
        Temporal,
        "score_seed",
        "Selects the Score's state arrangement. Authored identity, recalled as an endpoint rather than blended.",
    ),
    entry(
        Temporal,
        "score_state_count",
        "How many states the Score cycles through.",
    ),
    entry(
        Temporal,
        "score_trigger",
        "What advances the Score: a boundary, a downbeat, an audio onset, or a manual trigger.",
    ),
    entry(
        Temporal,
        "score_loop_driver",
        "Which live layer's loop boundary drives the Score, or none.",
    ),
    entry(
        Temporal,
        "reset_loop_boundary",
        "What the Score clears when a loop boundary passes.",
    ),
    entry(
        Temporal,
        "reset_downbeat",
        "What the Score clears on a downbeat. This is an imperative choice rather than a value, so nothing modulates it.",
    ),
    entry(
        Temporal,
        "disp_il_amount",
    ...
    entry(
        Temporal,
        "disp_phos_g",
        "Green phosphor persistence, the longest of the three in the P22 default.",
    ),
    entry(
        Temporal,
        "disp_phos_b",
        "Blue phosphor persistence, the shortest of the three.",
    ),
    entry(
        Temporal,
        "disp_model",
    ...
    entry(
        Temporal,
        "disp_scanlines",
        "How pronounced the scanline gaps are. It acts only under a non-flat display model, because a flat screen has no scan.",
    ),
    entry(
        Temporal,
        "disp_beam_width",
    ...
    entry(
        Temporal,
        "disp_beam_shape",
        "How sharply the beam falls off from its centre.",
    ),
    entry(
        Temporal,
        "disp_mask_strength",
        "How strongly the selected mask pattern shows.",
    ),
    entry(
        Temporal,
        "disp_mask_dark",
        "How dark the gaps between mask elements sit.",
    ),
    entry(
        Temporal,
        "disp_bloom",
        "How much bright areas spill into their surroundings.",
    ),
    entry(
        Temporal,
        "disp_bloom_radius",
        "How far the bloom spreads, over a fixed twelve-tap gather ring.",
    ),
    entry(
        Temporal,
        "disp_halation",
        "Adds the faceplate tint real glass gives to bloomed highlights.",
    ),
    entry(
        Temporal,
        "disp_defocus",
        "Softens the whole picture the way a misconverged or tired tube does.",
    ),
    entry(
        Temporal,
        "disp_sag",
        "Bows the picture geometry, measured at the picture centre, the way high-voltage sag pulls a CRT raster.",
    ),
    entry(
        Temporal,
        "melt_amount",
    ...
    entry(
        Temporal,
        "melt_width",
        "How wide a band around each coverage boundary the melt acts in.",
    ),
    entry(
        Temporal,
        "melt_hold",
    ...
    entry(
        Temporal,
        "melt_swirl",
        "Rotates the drag direction away from the boundary normal, up to a quarter turn either way.",
    ),
    entry(
        Temporal,
        "melt_chroma",
        "Lets colour run further off the edge than luma, by taking its chroma from a farther tap.",
    ),
    entry(
        Temporal,
        "melt_creep",
        "Biases the melt onto the uncovered side, so a keyed shape bleeds outward into the background rather than the background eating the shape.",
    ),
    entry(
        Temporal,
        "mosh_amount",
    ...
    entry(
        Temporal,
        "mosh_hold",
        "Re-applies the same delta several times under fresh timestamps, so motion keeps smearing in one direction.",
    ),
    entry(
        Temporal,
        "mosh_drop",
        "Starves the decoder of chunks. The last decoded picture is held, so a dropped chunk smears rather than flashing.",
    ),
    entry(
        Temporal,
        "mosh_shuffle",
        "Re-injects an older chunk out of order. Only chunks at least six deep are eligible, so the result is stale motion rather than a stutter.",
    ),
    entry(
        Temporal,
        "mosh_rate",
        "Scales how often hold, drop, and shuffle fire. It deliberately does not scale key removal.",
    ),
    entry(
        Temporal,
        "mosh_bitrate_starve",
        "Squeezes the encoder's bitrate. Every reconfigure forces a full re-acquire, so the picture snaps back and starts falling apart again.",
    ),
    entry(
        Temporal,
        "mosh_resync",
        "How often the encoder is forced to send a fresh key. At zero the picture never recovers on its own.",
    ),
    entry(
        Temporal,
        "mosh_recycle",
        "Feeds the encoder its own previous blended output instead of the clean image, so every pass builds on the last one's wreckage.",
    ),
    entry(
        Temporal,
        "sync_amount",
    ...
    entry(
        Ntsc,
        "enabled",
        "Runs composite VHS once over the finished programme, independently of per-layer Master bypass. It follows Codec Mosh in their shared latest-only worker hop (Mosh then VHS), with no backlog or second worker; completion latency depends on output size, settings, and hardware.",
    ),
    entry(
        Ntsc,
        "tape_speed",
        "Which tape speed is being emulated. Slower speeds carry less bandwidth and degrade more.",
    ),
    entry(
        Ntsc,
        "composite_noise_intensity",
        "How much noise rides on the composite signal as a whole.",
    ),
    entry(
        Ntsc,
        "composite_sharpening",
        "The sharpening a composite decoder applies, which is also what gives composite its characteristic ringing.",
    ),
    entry(
        Ntsc,
        "luma_noise_intensity",
        "How much noise sits on the brightness signal.",
    ),
    entry(
        Ntsc,
        "luma_smear",
        "How far brightness smears to the right of an edge, the way limited luma bandwidth smears it.",
    ),
    entry(
        Ntsc,
        "chroma_noise_intensity",
        "How much noise sits on the colour signal.",
    ),
    entry(
        Ntsc,
        "chroma_loss",
        "How often colour is lost outright, leaving stretches of the line monochrome.",
    ),
    entry(
        Ntsc,
        "snow_intensity",
        "Sparse dropout speckle across the picture.",
    ),
    entry(
        Ntsc,
        "head_switching_enabled",
        "Arms the tear at the bottom of the frame where a helical-scan head hands over to the next.",
    ),
    entry(
        Ntsc,
        "head_switching_height",
        "How many lines the head-switching tear occupies.",
    ),
    entry(
        Ntsc,
        "head_switching_shift",
        "How far the head-switching band is displaced sideways.",
    ),
    entry(
        Ntsc,
        "tracking_noise_enabled",
        "Arms the mistracking band, the noise a deck shows when the tape path is off.",
    ),
    entry(
        Ntsc,
        "tracking_noise_height",
        "How tall the mistracking band is. It sits low in the frame because that is where a deck loses lock on the control track first.",
    ),
    entry(
        Ntsc,
        "tracking_noise_wave",
        "How much the mistracking band waves rather than sitting straight.",
    ),
    entry(
        Ntsc,
        "tracking_noise_snow",
        "How much speckle the mistracking band carries.",
    ),
    entry(
        Ntsc,
        "edge_wave_enabled",
        "Arms a horizontal wobble on the whole picture, as an unstable timebase produces.",
    ),
    entry(
        Ntsc,
        "edge_wave_intensity",
        "How far the edge wave displaces each line.",
    ),
    entry(
        Ntsc,
        "edge_wave_speed",
        "How quickly the edge wave travels down the frame.",
    ),
];
