...
impl CompositionHost {
    ...
    fn encode_temporal_impl(
        ...
        if let Some((routed, current_view)) = routed {
            ...
            // Loom, Atlas, long exposure, and an authored B12 time-displace
            // map/interp all need the originals shader ahead of the routed
            // Garden pass; the slit lanes only matter while slit-scan is
            // active, mirroring the plan's own activity predicate.
            let pre_garden_originals_active = pre_garden_originals.loom_values[0] > 0.0
                || pre_garden_originals.atlas_values[0] > 0.0
                || pre_garden_originals.long_exposure_values[0] > 0.0
