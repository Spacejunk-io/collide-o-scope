...
impl VisualRack {
    ...
    /// Every group a node routes to. The fixed-width slot array keeps a
    /// multi-route kind visible: a `filter_map` closure structurally cannot
    /// yield a node's second route.
